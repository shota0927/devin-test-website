...
import { getRelatedVideos, Video } from '../api/videoApi';
...
export function VideoPage() {
  ...
  const videoFromContext = videos.find(v => v.id === videoId) || null;
  ...
  const isLoading = videosLoading || (relatedLoading && !relatedVideos.length);
  
  useEffect(() => {
    const fetchRelatedVideos = async () => {
      try {
        setRelatedLoading(true);
        const relatedVideosData = await getRelatedVideos(videoId);
  ...
  useEffect(() => {
    ...
    const handleVideoPlay = async () => {
      if (!viewCountedRef.current) {
        try {
          console.log('VideoPage: Video played, incrementing views');
          const newViewCount = await incrementViews(videoId);
          console.log('VideoPage: Received new view count:', newViewCount);
  ...
  if ((videosError && !video) || (!video && !videosLoading)) {
    return (
      <div className="container mx-auto py-16 px-4 text-center">
        <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded">
          {videosError || 'Video not found'}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div>
          ...
          <div className="space-y-4">
            {relatedLoading && !relatedVideos.length ? (
              ...
              <p className="text-gray-500">
                {relatedError || 'No related videos found'}
              </p>
            )}
          </div>
        </div>
